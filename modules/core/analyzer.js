...
LinkJuice.Analyzer = (function () {
  ...
  async function startAnalysis(params, progressCallback = null) {
    try {
      ...
      if (progressCallback)
        progressCallback({ step: "sitemap", message: "Parsing du sitemap..." });
          ...
          try {
            ...
            for (const link of result.links) {
              if (!crawledUrls.has(link) && !crawlQueue.includes(link)) {
                ...
                if (
                  domainFilterList.length === 0 ||
                  domainFilterList.some((domain) => link.includes(domain))
                ) {
      ...
      if (progressCallback)
        progressCallback({
          step: "orphaned",
          message: "Identification des pages orphelines...",
        });
      ...
      const anchorQuality =
        LinkJuice.AnchorQuality.analyzeAnchorQuality(linkContextData);
      ...
      const inTextAnalysis =
        LinkJuice.InTextAnalysis.analyzeInTextLinks(linkContextData);
      const pageRank = LinkJuice.PageRank.calculatePageRank(linkContextData);
      const juiceLinkScores =
        LinkJuice.JuiceLinkScore.calculateJuiceScores(linkContextData);
    ...
    "0.00%",
      ...
      const performanceMetrics =
        LinkJuice.PerformanceMetrics.generatePerformanceMetrics(
      ...
      const recommendations =
        LinkJuice.Recommendations.generateRecommendations(recommendationsData);
      ...
      analysisResults = {
        ...
        orphanedPercentage:
          sitemapUrls.length > 0
            ? parseFloat(
                (orphanedPages.length / sitemapUrls.length) * 100
              ).toFixed(2) + "%"
            : "0.00%",
  ...
  function generateLinksSummary() {
    ...
    if (summary.totalLinks > 0) {
      summary.byImportancePercentage = {
        high: ((summary.byImportance.high / summary.totalLinks) * 100).toFixed(
          1
        ),
        medium: (
          (summary.byImportance.medium / summary.totalLinks) *
          100
        ).toFixed(1),
  ...
  function getTopImportantPages(limit = 10) {
    ...
    for (const url in linkContextData) {
      ...
      const avgImportance =
        links.length > 0 ? totalImportance / links.length : 0;
      pageImportance[url] = {
        url: url,
  ...
  function getDashboardData() {
    if (!analysisResults) {
      return {
        success: false,
        message:
          "Aucune donnée d'analyse disponible. Veuillez d'abord lancer une analyse.",
      };
    }
    ...
    // Vérifier si linkStatusData existe et contient des données
    const hasLinkStatusData =
      analysisResults.linkStatusData &&
      Object.keys(analysisResults.linkStatusData).length > 0;

    console.log("????????????????? Statut des données linkStatusData:", {
      existe: !!analysisResults.linkStatusData,
      nombreEntrees: analysisResults.linkStatusData
        ? Object.keys(analysisResults.linkStatusData).length
        : 0,
      estTransmis: hasLinkStatusData,
    });
    ...
    // NOUVEAU: Traiter les données de contexte pour décoder les entités HTML
    let decodedLinkContextData = analysisResults.linkContextData;

    // Utiliser le décodeur si disponible
    if (
      typeof LinkJuice.TextUtils !== "undefined" &&
      typeof LinkJuice.TextUtils.decodeLinkContextTextData === "function"
    ) {
      decodedLinkContextData = LinkJuice.TextUtils.decodeLinkContextTextData(
        analysisResults.linkContextData
      );
      console.log("Décodage des entités HTML appliqué aux données de contexte");
    }

    // Créer un objet avec toutes les données pour le dashboard
    const dashboardData = {
      ...analysisResults,
      linkContextData: decodedLinkContextData, // Utiliser les données décodées
      metadata: metadata,
      linkStatusData: analysisResults.linkStatusData || {}, // S'assurer que les données de statut sont présentes
      importanceAnalysis: {
    ...
    console.log(
      "Transmission de linkStatusData au dashboard:",
      dashboardData.linkStatusData
        ? Object.keys(dashboardData.linkStatusData).length
        : 0,
      "entrées"
    );
